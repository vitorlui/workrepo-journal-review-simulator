...
def _reviewer_index(profile_id: str) -> int:
    return {
        "reviewer-methodology": 1, "reviewer-domain": 2,
        "reviewer-systems-architecture": 3, "reviewer-reproducibility": 4,
        "reviewer-scientific-impact": 5,
    }.get(profile_id, 9)


def save_generated_response(
    review_id: str, *, venue_id: str, reviewer_profile: str, engine: str,
    content: str, model: str = "", error: str = "",
) -> ResponseImport:
    """Persist an engine-generated review (from the Execute-query button).

    Stored with the same scheme as a manual upload so it shows up in the index
    and the DB exactly like an imported external response.
    """
    n = _reviewer_index(reviewer_profile)
    venue_dir = review_dir(review_id) / "external_responses" / "by_venue" / (venue_id or "UNKNOWN")
    venue_dir.mkdir(parents=True, exist_ok=True)
    fname = f"{review_id}__{venue_id}__rev{n}_{reviewer_profile}_{engine}_response.md"
    stored = venue_dir / fname

    header = (
        f"<!-- generated by CLI engine '{engine}'"
        f"{(' model=' + model) if model else ''}"
        f"{(' ERROR=' + error) if error else ''} at {now_iso()} -->\n\n"
    )
    write_text(stored, header + (content or "(empty response)"))

    has_sources = bool(re.search(r"https?://", content or ""))
    looks_incomplete = bool(error) or len((content or "").strip()) < 200
    ...
    append_line(index, f"| {venue_id} | {reviewer_profile} | {engine} (cli) | {stored.name} | {has_sources} | {looks_incomplete} |")

    try:
        from worker.db import ExternalResponse, session_scope
        with session_scope() as s:
            s.add(ExternalResponse(
                review_id=review_id, venue_id=venue_id, reviewer_profile=reviewer_profile,
                engine=engine, response_file_path=str(stored), has_sources=has_sources,
                looks_incomplete=looks_incomplete, status="generated" if not error else "error",
            ))
    except Exception:
        pass

    return ResponseImport(review_id, venue_id, reviewer_profile, engine, str(stored),
                          has_sources, looks_incomplete, [error] if error else [])
